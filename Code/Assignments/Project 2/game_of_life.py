...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(self.width):
            for y in range(self.height):
                pos = Point(x, y)
                block = Block(pos, 'red')
                self.block_list.update({'(%d, %d)' % (x, y): block})
    ...
    def seed(self, block_coords):
        ...
        for coord in block_coords:
            self.block_list['(%d, %d)' % (coord[0], coord[1])].set_live(self.canvas) 
    ...
    def get_block_neighbors(self, block):
    	...
    	for x in range(max(0, block.x -1), min(block.x + 1, self.width - 1) + 1):
    		for y in range(max(0, block.y - 1), min(block.y + 1, self.height -1) + 1):
    			if(x != block.x or y != block.y):
    				block_list.append(self.block_list['(%d, %d)' % (x, y)])
    ...
    def simulate(self):
        ...
        for x in range(self.width):
        	for y in range(self.height):
        		neighbor_num = 0
        		center_block = self.block_list['(%d, %d)' % (x, y)]
        		neighbors = self.get_block_neighbors(center_block)
        		
        		for block in neighbors:
        			if block.status == 'live':
        				neighbor_num += 1
				
				if center_block.status == 'live':
					if neighbor_num < 2 :
						center_block.new_status = 'dead'
					elif neighbor_num == 2 or neighbor_num == 3:
						center_block.new_status = 'live'
					elif neighbor_num > 3:
						center_block.new_status = 'dead'

				elif center_block.status == 'dead':
					if neighbor_num == 3:
						center_block.new_status == 'live'	
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                center_block = self.block_list['(%d, %d)' % (x, y)]
                center_block.reset_status(self.canvas)				 
...
if __name__ == '__main__':    
    ...
    board.random_seed(0.45)
    win.after(1000, board.animate)
